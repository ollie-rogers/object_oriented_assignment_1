...
bool choose_loop() {
    bool checker_5 = false; // assigns a variable to false for looping process
    std::string repeat; // creates a string variable
    while (checker_5 == false) {
        // asks user whether to repeat
        std::cout << "Would you like to repeat? (y/n)" << std::endl; 
        std::cin >> repeat; // assigns input to repeat variable
         // checks if input is of form given
        checker_5 = all_strings_validator(repeat, 2);
    }
    if (repeat == "y") { // if chose to repeat
        return false; // doesn't break the loop
    } else { // if chose not to repeat
